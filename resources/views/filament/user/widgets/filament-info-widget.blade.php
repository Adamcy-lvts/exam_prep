    ...
    <x-filament::section class="bg-white dark:bg-gray-800 rounded-lg shadow">
        <div class="flex items-center justify-between">
            <div class="flex flex-col">
                <span class="text-sm font-medium text-gray-600 dark:text-gray-300">Subscription Plan:
                    {{ optional($latestSubscription->plan)->title ?? 'N/A' }}</span>
                <!-- Replace with dynamic content -->
                <span
                    class="text-md font-semibold text-gray-800 dark:text-white">Expires on: {{ Carbon\Carbon::parse($latestSubscription->ends_at)->format('d-M-Y') }}</span>
            </div>
