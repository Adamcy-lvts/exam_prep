<x-filament-panels::page>
    ...
    <div class="text-center">
        <!-- Animated Checkmark -->
        <svg class="w-16 h-16 text-green-500 mx-auto animate-checkmark" xmlns="http://www.w3.org/2000/svg" fill="none"
    ...
    <!-- Information Columns -->
    <div class="mt-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 text-center text-gray-800 dark:text-gray-200">
        <!-- Total Questions -->
        ...
        <div class="p-12 rounded-lg shadow-xl transition-all duration-300 ease-in-out transform hover:scale-105 bg-white dark:bg-gray-800 backdrop-blur-md hover:backdrop-blur-lg flex justify-center items-center"
            ...
            <div>
                ...
                <p class="text-3xl font-bold text-green-500 dark:text-green-400">{{ $totalScore }}/{{ $totalMarks }}
                </p>
            </div>
        </div>
        <!-- Time Spent -->
    ...
    <div class="mt-6 text-center">
        ...
        <p class="text-lg dark:text-gray-300">
            Congratulations on completing the {{ $course->course_code }} exam! Your perseverance has paid off, and
            you've gained valuable
            knowledge. Review the topics you excelled at and those requiring more attention to reinforce your learning.
        </p>
    </div>
    ...
    {{-- <div class="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8">
        ...
        <div class="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-xl transition duration-300">
            ...
            <div>
                @foreach ($topicsDidWell as $performance)
                    @if ($performance->topic->unit && $performance->topic->unit->module)
                        <div class="text-lg font-bold">{{ $performance->topic->unit->module->name }}</div>
                    @endif
                    @if ($performance->topic->unit)
                        <div class="text-md font-semibold">{{ $performance->topic->unit->name }}</div>
                    @endif
        ...
        <div class="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-xl transition duration-300">
            ...
            <div>
                @foreach ($topicsDidNotDoWell as $performance)
                    @if ($performance->topic->unit && $performance->topic->unit->module)
                        <div class="text-lg font-bold">{{ $performance->topic->unit->module->name }}</div>
                    @endif
                    @if ($performance->topic->unit)
    ...
    <div x-data="{ open: false }" class="w-full mx-auto mt-16">
        ...
        <button @click="open = !open"
            ...
            <!-- Icon indicating the state of the accordion -->
            <svg :class="{ 'transform rotate-180': open }"
                class="w-4 h-4 md:w-6 md:h-6 ml-1 md:ml-2 transition-transform duration-300 ease-in-out" fill="none"
    ...
    {{-- <div class="w-full  mx-auto mt-16">
        <div class="bg-white dark:bg-gray-900 rounded-lg shadow-xl px-8 py-6 transition duration-300 ease-in-out">
            <h1 class="text-3xl font-bold mb-6 text-gray-900 dark:text-white">Answers</h1>
