<x-filament-panels::page>
    ...
    <div x-data="{ selectedQuestion: null }" class="bg-gray-100 dark:bg-gray-800 min-h-screen flex flex-col">
        ...
        <div class="flex flex-1 overflow-hidden">
            ...
            <div class="hidden md:block w-64 bg-gray-800 overflow-hidden flex-shrink-0">
                    ...
                    <div class="flex-1 overflow-y-auto py-4 px-2">
                        @foreach ($allquestions as $key => $q)
                            ...
                            <a href="{{ route('filament.user.resources.subjects.questions', ['record' => $quizzable->id, 'quizzableType' => $quizzableType, 'page' => $pageNumber]) . '#q' . ($key + 1) }}"
                                class="block mb-1 px-3 py-2 text-sm font-medium rounded-md transition-colors duration-150 ease-in-out
                                      {{ $loop->iteration == ($questions->currentPage() - 1) * 5 + 1 ? 'bg-gray-700 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white' }}
                                      {{ $isAnswered ? 'bg-green-800 text-green-200' : '' }}">
                                Q{{ $key + 1 }}
                                @if ($isAnswered)
                                    <svg class="inline-block ml-2 h-4 w-4 text-green-500" fill="none"
