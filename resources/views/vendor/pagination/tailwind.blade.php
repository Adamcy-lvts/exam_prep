@if ($paginator->hasPages())
    <nav role="navigation" aria-label="{{ __('Pagination Navigation') }}"
        ...
        {{-- Results Info --}}
        <div class="text-sm text-gray-900 dark:text-gray-400 leading-5 mb-2 sm:mb-0">
            Showing
                ...
                @for ($i = $start; $i <= $end; $i++)
                    @if ($i == $paginator->currentPage())
                        <span aria-current="page"
                            class=" relative items-center px-4 py-2 text-sm font-medium text-white bg-green-600 border border-green-700 cursor-default leading-5">
                            {{ $i }}
                        </span>
                    @else
