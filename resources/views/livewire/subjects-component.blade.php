        ...
        @foreach ($this->questions as $key => $question)
            ...
            <div class="mb-6">
                <p class="text-sm sm:text-lg text-gray-800 dark:text-gray-200 font-semibold mb-3 hover:text-green-600">
                    <span class="mr-2">Q{{ $questionNumber }}.</span>{{ $question->question }}
                </p>
                ...
                @foreach ($question->options as $option)
                    <label
                        ...
                        <span class="text-gray-700 dark:text-gray-300 text-sm sm:text-lg"><span class="mr-3">{{ chr($loop->index + 65) }}.</span>
                            {{ $option->option }}</span>
                    </label>
                @endforeach
            </div>
